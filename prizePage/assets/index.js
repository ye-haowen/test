(() => {
  ...
  function countDown() {
    requestAnimationFrame(() => {
      ...
      if (pageData.beginTime) {
        let time = pageData.scrollTime - Math.ceil((new Date().getTime() - pageData.beginTime) / 1000)
        time = time > 0 ? time : 0
  ...
  function scrollView() {
    window.requestAnimationFrame(() => {
      let top = parseInt(pageData.scrollEl.style.top) || 0
  ...
  function getPrizeUser() {
    ...
    document.querySelector(".prizeLine")?.classList.remove("prizeLine")
